...
覆盖:
  - strip_thinking_tags: step-3.7-flash 等模型把思考过程作为普通 text 输出
    包裹在 <think>...</think> 里, 需要在返回频道前剥掉
  - strip_model_artifacts: 组合入口, 一次剥掉 tool_call XML + thinking 标签
"""

from __future__ import annotations

from mmag.model_artifacts import (
...
class TestStripThinkingTags:
    ...
    def test_only_think_returns_empty(self):
        ...
        out = strip_thinking_tags("<think>只有内心独白</think>")
        assert out == ""
...
class TestStripToolCallXml:
    """对称覆盖 - 保证 strip_tool_call_xml 行为没回归(只动 strip_model_artifacts 的话)"""

    def test_simple_tool_call_block_removed(self):
        text = '<tool_call>{"name": "x"}</tool_call>好的'
        out = strip_tool_call_xml(text)
        assert "<tool_call>" not in out
        assert "好的" in out

    def test_no_tag_unchanged(self):
        assert strip_tool_call_xml("hello") == "hello"

    def test_sdk_invoke_block_removed(self):
        text = '<invoke name="search">{"query":"x"}</invoke>完成'
        assert strip_tool_call_xml(text) == "完成"
...
class TestStripModelArtifacts:
    ...
    def test_think_only(self):
        ...
        assert strip_model_artifacts(text) == "公开答复"
    ...
    def test_both_think_and_tool_call_removed(self):
        ...
        out = strip_model_artifacts(text)
        assert "<think>" not in out
        assert "<tool_call>" not in out
    ...
    def test_only_artifacts_returns_empty(self):
        ...
        assert strip_model_artifacts(text) == ""
    ...
    def test_order_does_not_matter_for_disjoint_blocks(self):
        ...
        assert strip_model_artifacts(a) == strip_model_artifacts(b) == "好"
