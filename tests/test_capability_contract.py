...
def test_builtin_catalog_has_one_runtime_projection():
    names = [tool.name for tool in build_builtin_bindings(_client(), MagicMock())]
...
async def test_binding_validates_input_and_returns_stable_error():
    result = await bind_langgraph_capability(
        create_get_channel_info_capability(_client())
    ).handler()
...
async def test_search_knowledge_applies_schema_default_and_limit():
    ...
    result = await bind_langgraph_capability(spec).handler(
...
async def test_get_posts_uses_memory_without_rest_side_effects():
    ...
    result = await bind_langgraph_capability(
        create_get_posts_capability(client, memory)
    ).handler(channel_id="channel-1")
...
async def test_analyze_link_adds_structured_source_metadata(monkeypatch):
    ...
    result = await bind_langgraph_capability(spec).handler(url="https://example.com/docs")
...
async def test_write_policy_stops_side_effect(authorization, status):
    ...
    result = await tool.handler(channel_id="channel-1", key="k", value="v")
