...
- `get_channel_info`、`search_knowledge` 已迁移为单一 `CapabilitySpec`，不再重复 schema/handler/formatter；其余内置工具仍在 `tools/builtin.py` 与 `sdk_tools.py` 双重定义
...
| 2026-07-31 | CI / `make verify` 工程门禁：Ruff、225 个离线测试、47.95% 分支覆盖率、mypy、wheel smoke |
