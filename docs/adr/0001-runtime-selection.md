# ADR-0001：LangGraph 作为默认 Agent Runtime

- 状态：Superseded（2026-07-31 修订）
- 日期：2026-07-31
...
3. 需要审批的 Capability 在 `review_tools` 节点调用原生 `interrupt()`；批准、修改参数或拒绝后，用同一 `thread_id` 和 `Command(resume=...)` 恢复。
4. 审批必须发生在工具副作用之前。策略评估保持纯函数，业务审批单与审计状态由 control plane 持久化。
5. Claude Agent SDK 保留为 `USE_SDK_LLM=true` 的显式可选后端，不是默认后端；需要可恢复人工审批的部署不得切换到该路由。
6. Run 内禁止自动跨后端重试，避免外部写入被重复执行。

## 后果

- LangGraph checkpoint 是执行状态的单一事实来源，control plane 是任务、审批和审计状态的单一事实来源。
- `LegacyRuntimeAdapter` 被移除；默认值改为 `USE_SDK_LLM=false`。
