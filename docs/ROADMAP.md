...
演示体验已覆盖 Slash Command 能力发现、个人资产、结果按钮、真人审批和附件预览；命令与阅读使用
2 倍速，普通流程前 80% 8 倍、后 20% 3 倍，关键按钮保持正常速度。旁白覆盖全部业务章节并统计实际
有声占比；旁白按完整语义短句逐句调用 MMX，句间保留受控停顿；字幕默认最多 18 字、禁止机械截断，
展示时移除末尾句号，封面不显示字幕。
...
- [x] Bot Token 只代表服务身份，不继承为发消息用户的权限；Artifact 读取、Checkpoint 恢复、审批和交付
  已通过 Mattermost 成员关系及 MMAG Policy 做动态影子授权，授权查询失败时默认拒绝；
- [ ] `post_edited`、`post_deleted` 已更新或撤销消息、FTS 和摘要有效状态；继续消费成员关系和用户状态
  事件，并将撤销传播到派生知识、Artifact 和待恢复任务；MMAG 保留策略不得绕过 Mattermost/企业数据
  保留要求；
- [x] 个人 Artifact 默认只允许交付到本人 DM；
- [ ] 发布个人 Artifact 到项目或频道时创建带 provenance 的新版本并显式审批，不原地放宽原对象 Scope；
...
- [x] 已用真实 Mattermost 账号完成个人 Skill 查看、选择、运行与 WorkCase 候选生成，并纳入四场景录制。
...
一次性 Token 并记录 Persona 版本、Hash 和审计。代答请求现已接入独立 Outbox，支持超时关闭、进程重启后
恢复已批准交付、失败后由所有者重试；策略编辑会生成新的不可变草稿 revision，重新发布后才影响新请求。
仍需扩展文档/WorkCase 数据源、成员与用户状态撤销传播、集中式运维查询，以及持续反馈闭环。

- [x] 已由第二位真实用户询问所有者发布的 Persona，并验证授权快照、代理身份和原 Thread 回答。
...
- [x] 首批显式总结链路：在 @Bot 请求中识别线程/近期讨论总结，由 Report Agent 选择 `meeting` Skill；
  服务端绑定当前频道和时间/线程范围，决定、待办和开放问题保留 Mattermost Post ID 来源。
- [x] 已在一次性私有三人频道中用真实消息验证 `Report Agent → meeting Skill → get_posts → 来源化结果`。
- [ ] 后续增加无 @ 的主动介入策略、长期会议对象和结果沉淀；普通频道消息不得自动扩大历史读取。
...
- [x] 演示闭环已验证 PPT Agent 在受控 Host Workspace 中生成预览和可编辑 PPTX，经真人原生按钮批准后外发；
  该链路证明通用 HITL 与 Artifact 交付可用，但不等同于领域业务对象和组织审批链已经完成。
