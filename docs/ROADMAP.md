...
- [x] 默认离线测试基线达到 `227 passed, 2 deselected`，实际分支覆盖率 `48.81%`；
...
3. [ ] 迁移 `search_messages` 和 `get_user_profile`，收口多条件过滤、组合读取与结果格式；
4. [ ] 迁移 `analyze_link`，让 `SourcePolicy.AUTO` 真正驱动来源注入；
5. [ ] 迁移 `save_knowledge`，为 `WRITE` 能力接入确定性的权限检查和未来审批钩子；
6. [ ] 单独处理 `send_file`：它依赖当前消息意图和文件边界，应与 Step 4 的不可变 `RunContext` 一起消除全局 `ToolContext.current_post`；
7. [ ] 最后让外部 MCP 进入同一 Catalog/Policy 可见性链路，并删除两套重复工厂与 formatter。
